...
char device_id[] = 		"fec58c053b124de2b42ad940570c5fab";
char device_token[] = 	"69cfc8eed1994731b1cde6bb0d17e777";
...
void onMessage(void *client, mqtt_msg_t *msg) {
    ...
    if (strncmp(strActName, "\"openWindow\"", 7) == 0) {
    	printf("Window is opened\n");
    	openWindow();
    	sprintf(buf,"{\"window\":1}");
    	char *p_msg = buf;
    	//gpio_write(48, 0);

    	mqtt_msg_t message;
    	message.payload = (char*)p_msg;
    ...
    } else if (strncmp(strActName, "\"closeWindow\"", 8) == 0) {
    	printf("Window is closed\n");
    	closeWindow();
    	sprintf(buf,"{\"window\":0}");
    	char *p_msg = buf;
    	//gpio_write(48, 0);
    	...
    	int ret = mqtt_publish(pClientHandle, message.topic, (char*)message.payload, message.payload_len, message.qos, message.retain);
    }else if (strncmp(strActName, "\"openFilter\"", 8) == 0) {
    	printf("Filter is opened\n");
    	openFilter();
    	sprintf(buf,"{\"filter\":1}");
    	char *p_msg = buf;
    	//gpio_write(48, 0);
    	...
    	int ret = mqtt_publish(pClientHandle, message.topic, (char*)message.payload, message.payload_len, message.qos, message.retain);
    }else if (strncmp(strActName, "\"closeFilter\"", 8) == 0) {
    	printf("Filter is closed\n");
    	closeFilter();
    	sprintf(buf,"{\"filter\":0}");
    	char *p_msg = buf;
    	//gpio_write(48, 0);
...
int* getADC(void)
{
	...
	while(1){
		...
		if(samples.am_channel==0){//미세먼지
			ADC[0]=samples.am_data;
		}if(samples.am_channel==1){//가스센서
			ADC[1]=samples.am_data;
		}if(samples.am_channel==2){//비감지센서
			ADC[2]=samples.am_data;
		}if(samples.am_channel==3){//조도센서
			ADC[3]=samples.am_data;
		}

		if(ADC[0]!=0 && ADC[1]!=0 && ADC[2]!=0 && ADC[3]!=0){
			close(fd);
			return ADC;
		}
...
int getWindowState(void){
	...
	char window_buf[4];
	memset(window_buf, 0, 4);
    int ret;
	int fd = open("/dev/gpio57", O_RDWR);

	ioctl(fd, GPIOIOC_SET_DIRECTION, GPIO_DIRECTION_IN);

   	//filter opened/closed 확인

	lseek(fd, 0, SEEK_SET);//파일을 읽기전에 파일의 포인터를 0으로 가리키게 하겠다.
	ret=read(fd, window_buf, sizeof(window_buf));

	//printf("%d", ret);
	close(fd);

	if (window_buf[0] == '1') return 1;
	else return 0;
...
int getFilterState(void){
	...
	lseek(fd, 0, SEEK_SET);//파일을 읽기전에 파일의 포인터를 0으로 가리키게 하겠다.
	ret=read(fd, filter_buf, sizeof(filter_buf));

	//printf("%d", ret);
	close(fd);

	if (filter_buf[0] == '1') return 1;
	else return 0;
...
int main(int argc, FAR char *argv[])
{
	...
	//윈도우 필터 상태 read
	int windowState=0;//getWindowState();
	int filterState=0;//getFilterState();
	char window[32];
	char filter[32];
	char buf[64];
	char* msg;
	...
	//int fd1=open("/dev/adc0", O_RDONLY);
	//ret=ioctl(fd1, ANIOC_TRIGGER, 0);
	...
	while(1){
    	...
    	if(windowState==0)//closed
    	{
    		sprintf(window,"opened");
    	}else sprintf(window, "closed");

       	if(filterState==0)//closed
		{
			sprintf(filter,"opened");
		}else sprintf(filter, "closed");



       	sprintf(buf, "{\"window\" : \"%s\" , \"filter\" : \"%s\"}",window, filter);
		msg=buf;
       	mqtt_msg_t message;
		message.payload = msg;
		message.payload_len = strlen(buf);
...
/*
		while(1){
			...
			gpio_write(V_LED, 0);
			usleep(280);
			nbytes=read(fd1, &samples, sizeof(samples));
			usleep(40);
			gpio_write(V_LED, 1);
			usleep(9680);
		...
		sprintf(buf, "{\"dust\" : %d, \"smoke\": %d, \"rain\": %d, \"brightness\" : %d }",
